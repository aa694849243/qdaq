...
readme: this function need to be update as encode method 2D values is not the same as red-ant
"""
...
def dict_to_xml(data, test_number, dO, revStep, revCount):
    """
    功能： 指定测试段的结果数据转XML（二维数据进行重编码处理，匹配了AQS的XML报告），
    输入：指定测试段
    1. 结果数据
    2. 测试段索引
    3. 阶次谱分辨率
    ...
    def base64_converter(data):
        ...
        binary_stream.write(struct.pack('<i', len(data)))
        for x in data:
            binary_stream.write(struct.pack('<d', x))
        binary_stream.seek(0)
        return base64.b64encode(binary_stream.read()).decode("utf-8")
...
if __name__ == '__main__':
    ...
    try:
        path_to_json = r'D:\aqs2rt\temp\test_9_201028062430.json'
        path_to_xml = r'D:\aqs2rt\temp\test3.xml'
