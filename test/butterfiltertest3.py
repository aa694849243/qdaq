...
if __name__ == "__main__":
    ...
    freq_nested_list=list()
    freq_list1=np.linspace(100,2000,1901,endpoint=True,dtype="i")
    # freq_list2=np.linspace(500,1500,501,endpoint=True,dtype="i")
    # freq_list3=np.linspace(800,1200,201,endpoint=True,dtype="i")
    freq_nested_list.append(freq_list1)
    # freq_nested_list.append(freq_list2)
    # freq_nested_list.append(freq_list3)
    ...
    max_value_dict=dict()
    order_list=[3,9]
    for i, freq_list in enumerate(freq_nested_list):
        ...
        for j,order in enumerate(order_list):
            for coff in coff_list:
                ...
                plt.plot(freq_list,fft_abs[freq_list],c=["r","b","g"][j],label="freqlist:{}order:{}".format(i+1,order))
                plt.xlabel("freq")
                plt.grid()
