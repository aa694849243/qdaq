...
@Version: 1.0.0
reference: https://blog.csdn.net/sjt1996/article/details/83377800
"""
import rsa
...
class Rsaencrypt:
    def __init__(self, public_key):
        ...
        # 但是里面又有着11个字节是它必须有的, 所以最长只能加密117个字节
...
def save_pem(p_key, key_filename):
    """
    功能：保存公钥或私钥
    输入：
    1. 密钥
    ...
    返回：无
    """
    with open(key_filename, 'wb+')as fp:
        fp.write(p_key.save_pkcs1())
...
def read_key(key_filename, key_type=0):
    ...
    if key_type:
        # 读取私钥
        p_key = rsa.PrivateKey.load_pkcs1(p_info)
    else:
        p_key = rsa.PublicKey.load_pkcs1(p_info)
    return p_key
...
if __name__ == '__main__':
    ...
    # 保存密钥
    pub_key_filename = r'D:\qdaq\test\210721-1\rsa_1\public.pem'
    pri_key_filename = r'D:\qdaq\test\210721-1\rsa_1\private.pem'
    # 保存密钥（包括公钥和私钥）
    save_pem(pubkey, pub_key_filename)
    save_pem(privkey, pri_key_filename)
    # 读取密钥（从已保存文件中读取）
    ...
    # 2. 加密后写入json文件
    # 初始化密钥
    en_config_filename = r'D:\qdaq\test\210721-1\rsa_1\en_config.json'
    t2 = time.time()
    # 读取密钥并生成相关方法
    pub_key = read_key(pub_key_filename)
    ...
    # 2. 加密后写入json文件
    en_limit_filename = r'D:\qdaq\test\210721-1\rsa_1\en_limit.json'
    t2 = time.time()
    # 读取密钥并生成相关方法
    pub_key = read_key(pub_key_filename)
    rsa_en_2 = Rsaencrypt(pub_key)
    ...
    # 使用不正确的密钥进行解密
    old_pub_key_filename = r'D:\qdaq\test\210721-1\rsa_1\public_old.pem'
    old_pri_key_filename = r'D:\qdaq\test\210721-1\rsa_1\private_old.pem'
