...
def single_resolver(speed_array, left_index, right_index, speed_last_frame, sampleRate, coils,
                    trigger_array, index_trigger, ppr):

    frame = speed_array[left_index:right_index]
    # 除第一帧，frame以上一帧最后一个零点的右中值跳跃点开始
    hSin = fftpack.hilbert(frame)
    envSin = np.sqrt(frame ** 2 + hSin ** 2)
    max_envSin = np.max(envSin)
    low_level = max_envSin / 7
    ...
    for i in range(len(half_loc_skip)):
        if (half_loc[half_loc_skip[i] + 1] - half_loc[half_loc_skip[i]]) > point_per_fish/4:
    ...
    envSin_filter = butter_filter(envSin, [int((speed_last_frame / 60) * 2.56 * coils)], sampleRate)
    ...
    return index_trigger, zeroLoc[-1]-int(point_per_fish/4) + left_index

def single_resolver_version2(speed_array, left_index, right_index, speed_last_frame, sampleRate, coils,
                    trigger_array, index_trigger, ppr):
    ...
    # 除第一帧，frame以上一帧最后一个零点的右中值跳跃点开始
    # frame_filter=butter_filter(frame,[int((10000+10000/60)*2.56)],sampleRate)
    hSin = fftpack.hilbert(frame)
    envSin = np.sqrt(frame ** 2 + hSin ** 2)
    max_envSin = np.max(envSin)
    low_level = max_envSin / 7
    ...
    for loc in min_array:
        envSin[loc:] *= -1
    # 滤波后截取可信的段
    envSin_filter = butter_filter(envSin, [int((speed_last_frame / 60) *2* 2.56 * coils)], sampleRate)[left_more:-right_more]
...
def single_resolver_version4(speed_array, left_index, right_index, speed_last_frame, sampleRate, coils,
                    trigger_array, index_trigger, ppr):
    # left_index为实际需要的段，滤波时要向左取帧长1/10
    # right_index为原始数据存储到的位置，左帧长1/10处在该帧滤波后的数据不使用
    right_more=int((right_index-left_index)/10)
...
def speed_calc_single_resolver(filename,channelName):
    ...
    speed_sin=allrawdata["AIData"][channelName][64*102400:65*102400]
    plt.plot(speed_sin)
    plt.show()
    speed_sin=allrawdata["AIData"][channelName]
...
def speed_calc_resolver2(filename):
    last_angle_l1f=0
    loc_l1f=0
    last_angle_l2f=0
    loc_l2f=0
    ...
    gv_dict_rawdata['Sin']=allrawdata["AIData"]["Sin"]

    gv_dict_rawdata['Cos']=allrawdata["AIData"]["Cos"]
...
if __name__ == "__main__":
    path=r"D:\qdaq\rawdata\PM61191\Data"
    filenames=os.listdir(path)
    # filenames=["resolvertest_210923083612.h5"]
    # path=r"d:\qdaq\simu"
    # filenames=["PM61191-2vibs.tdms"]
    pic_savepath="e:/resolver_compare/pic"
    for filename in filenames:
        ...
        try:
            for channelName in ["Sin","Cos"]:
                ...
                rpml_resolver[channelName],rpm_resolver[channelName]=speed_calc_single_resolver(os.path.join(path,filename),channelName)
                print(time.time()-time_start)
            rpml_resolver2,rpm_resolver2=speed_calc_resolver2(os.path.join(path,filename))
