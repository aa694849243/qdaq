...
# 读取基本信息
from parameters_for_const_recalc_old import basic_info_update, task_info_update, speed_calc_info_update, \
...
from indicator_tools_for_const_recalc_old import twod_time_domain_for_share, order_spectrum_for_const_fluctuation, \
    twod_stat_factor_for_const_fluctuation, oned_time_domain, average_oned_stat_factor, \
    convert_time_speed_for_const, twod_order_spectrum, cepstrum, oned_order_spectrum, db_convertion
from initial_old import create_empty_twodtd_for_share, create_empty_temptd_for_share, \
...
if __name__ == '__main__':
    ...
    # config["filename"] = "byd-alltests.h5"
    try:
            ...
            for sensor_index, chan_name in enumerate(taskInfo['targetChan']):
                    ...
                    if counter_data_queue == 0:
                        ...
                        twod_sf = create_empty_twodsf_for_const(statFactorCalcInfo, sensor_index,
                                                                ...
                                                                speedRecogInfo[
                                                                    'initial_indicator_diagnostic'][
                                                                    testNameIndex])
                        temp_sf = create_empty_tempsf_for_const(statFactorCalcInfo, max_circle)
                ...
                test_result['resultData'][sensor_index]['dataSection'][
                    ...
                    oned_time_domain(temp_td, calSize,
                                     timeDomainCalcInfo, sensor_index,
                                     db_flag=basicInfo['dBFlag']))
                ...
                test_result['resultData'][sensor_index]['dataSection'][0][
                    'onedData'].append(
                    {"name": "Speed", "unit": "", "value": np.mean(rpm),
                ...
                if test_result['resultData'][sensor_index]['dataSection'][
                    ...
                    test_result['resultData'][sensor_index]['dataSection'][
                        ...
                        'onedData'] = list()
                # 计算一维按圈计算指标并更新二维结果
                twod_sf, oned_sf = average_oned_stat_factor(twod_sf,
                                                            statFactorCalcInfo,
                                                            sensor_index)
                test_result['resultData'][sensor_index]['dataSection'][
                    0]['onedData'].extend(oned_sf)
                ...
                # 更新二维按圈计算指标的x轴
                twod_sf = convert_time_speed_for_const(twod_sf,
                                                       indicator_num=statFactorCalcInfo[
                                                           'indicatorNum'])
                # 添加二维按圈计算指标到二维时间域指标中
                test_result['resultData'][sensor_index]['dataSection'][0][
                ...
                test_result['resultData'][sensor_index]['dataSection'][
                    0]['twodCeps'] = \
                    cepstrum(twod_os, cepstrumCalcInfo, taskInfo,
                             sensor_index, db_flag=basicInfo['dBFlag'])
