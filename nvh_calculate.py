...
from common_info import max_size, flag_index_dict, save_type, qDAQ_logger, version, ramp_quality_flag, cut_off_freq
from initial import create_empty_twodtd_for_share, \
    create_empty_temptd_for_share, create_empty_twodsf_for_share, create_empty_tempsf_for_share, \
    update_nvh_data_for_thread, \
...
def nvh_calc_process_for_qdaq(in_Q, out_Q, gv_dict_status,
    ...
    index_twod_td = 0
    sensorNames = []
    for channel_index, channel_name in enumerate(param.taskInfo['channelNames']):
        if channel_name.lower()[:3] in ['mic', 'umi']:
            sensorNames.append('soundIndicatorList')
        elif channel_name.lower()[:3] == 'vib':
            sensorNames.append('vibrationIndicatorList')

    while gv_dict_flag[flag_index_dict['nvhCalclation']]:
        try:
            data = in_Q.get(block=True, timeout=1)  # 识别到了才开始计算
        except Empty:
            continue
        ...
        if data['sectionEnd']:
            ...
            qDAQ_logger.debug(f"sensor {sensor_index} nvh_get_sectionend:{time.time() - time_click_start}")

        if counter_data_queue == 0:
            qDAQ_logger.debug(f"sensor {sensor_index} nvh_start:{time.time() - time_click_start}")
            # 测试段的第一帧数据
            testSectionIndex = data['testNameIndex']
            # 左索引
            ...
            maxLen = int(maxT * sampleRate // calSizes[testSectionIndex]) + 1
            # 创建空的twodtd和空的temp_td，创建的容量保证能放下测试段内的数据，
            # 将来测试段结束后再进行截取，然后再进行计算
            twod_td = create_empty_twodtd_for_share(param.timeDomainCalcInfo[testSectionIndex], sensor_index,
                                                    sensorNames[sensor_index], maxLen,
            ...
            rspMaxLen = int(maxT * maxSpeed / 60 // param.orderSpectrumCalcInfo[testSectionIndex]['dr_af']) + 1
            ...
            twod_sf = create_empty_twodsf_for_share(param.statFactorCalcInfo[testSectionIndex], sensor_index,
                                                    sensorNames[sensor_index], rspMaxLen, indicator_diagnostic=
                                                    param.speedRecogInfo['initial_indicator_diagnostic'][
                                                        data['recogIndex']])
            temp_sf = create_empty_tempsf_for_share(param.statFactorCalcInfo[testSectionIndex], sensor_index,
                                                    sensorNames[sensor_index], rspMaxLen)

            # 每32圈计算一次，overlapRatio为0.75 即不仅长度为8
            osMaxLen = round(maxT * maxSpeed / 60 // (param.orderSpectrumCalcInfo[testSectionIndex]['revNum'] * (
                    1 - param.orderSpectrumCalcInfo[testSectionIndex]['overlapRatio']))) + 1
            threed_os = create_empty_threedos_for_share(param.orderSpectrumCalcInfo[testSectionIndex], param.taskInfo,
                                                        sensor_index, sensorNames[sensor_index], osMaxLen,
                                                        db_flag=param.basicInfo['dBFlag'],
                                                        indicator_diagnostic=param.speedRecogInfo[
                                                            'initial_indicator_diagnostic'][
                                                            data['recogIndex']])
            twod_oc = create_empty_twodoc_for_share(param.orderCutCalcInfo[testSectionIndex],
                                                    param.taskInfo,
        ...
        while (counter_ar + 1) * sampsPerChan < len_vib:
            if counter_ar > 0:
                ...
                s_index = counter_ar * sampsPerChan - arPoints[testSectionIndex] * 2
            else:
                # 第一帧无法往前取点
                s_index = 0
            # 确定结束点的索引
            ...
            try:
                vib_rsp_frame, vib_rsp_time_frame = angular_resampling(rev, rev_time, target_vib,
                                                                       ...
                                                                       arPoints[testSectionIndex],
                                                                       param.orderSpectrumCalcInfo[testSectionIndex][
                                                                           'dr_bf'],
                                                                       param.orderSpectrumCalcInfo[testSectionIndex][
                                                                           'dr_af'])
            except Exception:
        ...
        try:
            ...
            twod_td, temp_td, counter_td, index_twod_td, last_calc_index = twod_time_domain_for_share(
                ...
                last_calc_index,
                vib_right_index,
                calSizes[testSectionIndex],
                param.timeDomainCalcInfo[testSectionIndex][sensorNames[sensor_index]],
                param.timeDomainCalcInfo[testSectionIndex]["refValue"][sensor_index],
                twod_td,
                temp_td,
                counter_td,
                sampleRate,
        ...
        try:
            ...
            threed_os, twod_oc, counter_or = order_spectrum_for_share(threed_os,
                                                                      ...
                                                                      param.orderSpectrumCalcInfo[testSectionIndex],
                                                                      param.orderCutCalcInfo[testSectionIndex],
                                                                      sensor_index,
                                                                      db_flag=param.basicInfo['dBFlag'],
                                                                      cut_off_points=
                                                                      param.orderSpectrumCalcInfo[testSectionIndex][
                                                                          'cutOffNum'][data['testNameIndex']])
        except Exception:
            gv_dict_status["code"] = 3000
        ...
        try:
            ...
            twod_sf, temp_sf = twod_stat_factor_for_share(twod_sf, temp_sf, vib_rsp[:vib_rsp_index],
                                                          ...
                                                          sensor_index, sensorNames[sensor_index],
                                                          param.statFactorCalcInfo[testSectionIndex])
        except Exception:
            gv_dict_status["code"] = 3000
        ...
        if data['sectionEnd']:
            ...
            try:
                ...
                if test_result['resultData'][sensor_index]['dataSection'][data['testNameIndex']]['onedData'] is None:
                    test_result['resultData'][sensor_index]['dataSection'][data['testNameIndex']]['onedData'] = list()
                test_result['resultData'][sensor_index]['dataSection'][data['testNameIndex']]['onedData'].extend(
                    oned_time_domain(temp_td, calSizes[testSectionIndex], param.timeDomainCalcInfo[testSectionIndex],
                                     sensor_index, sensorNames[sensor_index],
                                     db_flag=param.basicInfo['dBFlag']))
                qDAQ_logger.info(
                    f"1D time domain indicators calculation of sensor{sensor_index + 1}, test: {param.speedRecogInfo['testName'][data['recogIndex']]} finished")
            except Exception:
                gv_dict_status["code"] = 3000
            ...
            try:
                ...
                #                                             sensor_index)
                oned_sf, twod_sf = oned_stat_factor_mean(temp_sf, twod_sf, param.statFactorCalcInfo[testSectionIndex],
                                                         sensor_index, sensorNames[sensor_index])
                test_result['resultData'][sensor_index]['dataSection'][data['testNameIndex']]['onedData'].extend(
                    oned_sf)
                qDAQ_logger.info(
                    f"1D indicators by revolution calculation of sensor{sensor_index + 1}, test: {param.speedRecogInfo['testName'][data['recogIndex']]} finished")
            except Exception:
                gv_dict_status["code"] = 3000
            ...
            test_result['resultData'][sensor_index]['dataSection'][data['testNameIndex']][
                'twodTD'] = twod_td
            qDAQ_logger.info(
                f"2D time domain indicators calculation of sensor{sensor_index + 1}, test: {param.speedRecogInfo['testName'][data['recogIndex']]} finished")
            # 更新二维按圈计算指标的x轴
            ...
            # 添加二维按圈计算指标到二维时间域指标中
            twod_sf_list = []
            for info in twod_sf.values():
                twod_sf_list.extend(info.values())
            test_result['resultData'][sensor_index]['dataSection'][data['testNameIndex']]['twodTD'].extend(twod_sf_list)
            qDAQ_logger.info(f"2D indicators by revolution calculation of sensor{sensor_index + 1} finished")
            # update stop and end time of present test into final test result
            test_result['resultData'][sensor_index]['dataSection'][data['testNameIndex']]['startTime'] = data[
                                                                                                             'vib_start_index'] / sampleRate
            test_result['resultData'][sensor_index]['dataSection'][data['testNameIndex']]['endTime'] = data[
                                                                                                           'vib_right_index'] / sampleRate
            try:
                # update 2D order spectrum into final test result
                twod_os = twod_order_spectrum(threed_os, param.taskInfo, sensor_index)
                qDAQ_logger.info(
                    f"2D order spectrum calculation of sensor{sensor_index + 1}, test: {param.speedRecogInfo['testName'][data['recogIndex']]} finished")
            except Exception:
                gv_dict_status["code"] = 3000
            ...
            try:
                ...
                test_result['resultData'][sensor_index]['dataSection'][data['testNameIndex']]['twodCeps'] = cepstrum(
                    twod_os, param.cepstrumCalcInfo[testSectionIndex], param.taskInfo,
                    sensor_index, db_flag=param.basicInfo['dBFlag'])
                qDAQ_logger.info(
                    f"2D order cepstrum calculation of sensor{sensor_index + 1}, test: {param.speedRecogInfo['testName'][data['recogIndex']]} finished")
            except Exception:
                gv_dict_status["code"] = 3000
            ...
            try:
                ...
                test_result['resultData'][sensor_index]['dataSection'][
                    data['testNameIndex']]['onedData'] += oned_order_spectrum(twod_os,
                                                                              param.onedOSCalcInfo[testSectionIndex],
                                                                              param.taskInfo,
            ...
            try:
                ...
                test_result['resultData'][sensor_index]['dataSection'][data['testNameIndex']][
                    'twodOS'] = twod_os
                qDAQ_logger.info(
                    f"order conversion of sensor{sensor_index + 1}, test: {param.speedRecogInfo['testName'][data['recogIndex']]} finished")
            except Exception:
                gv_dict_status["code"] = 3000
            ...
            qDAQ_logger.info(
                f"nvh calculation of sensor{sensor_index + 1}, test: {param.speedRecogInfo['testName'][data['recogIndex']]} finished ")
    ...
    while gv_dict_flag[flag_index_dict['nvhCalclation']]:
        ...
        try:
            ...
            while vib_start_index + (counter_or + 1) * sampsPerChan <= vib_right_index:
                ...
                # 计算阶次谱需要滤除的低阶成分的点数
                cut_off_points = round(
                    cut_off_freq * 60 / speed / param.orderSpectrumCalcInfo['orderResolution'])
                # print("cut off points：{}, speed: {}".format(cut_off_points, speed))
                # 阶次谱计算
        ...
        if data['sectionEnd']:
            ...
            try:
                ...
                oned_sf, twod_sf = oned_stat_factor_mean_for_const(temp_sf, twod_sf, param.statFactorCalcInfo,
                                                                   sensor_index)
                test_result['resultData'][sensor_index]['dataSection'][
            ...
            qDAQ_logger.debug(
                "nvh获取到测试段{}最后一帧到已经将数据放入queue:{}".format(data["testNameIndex"], time.time() - time_get_sectionend))
            qDAQ_logger.debug(
                "拿到测试段{}第一帧到已经将数据放入queue:{}".format(data["testNameIndex"], time.time() - time_get_sectionstart))
            profiler.stop()
