...
from parameters import task_info_update
from mic_calibrate import cal_sensity
...
def speed_process(Q_speed_in, Q_speed_out, Q_speed_nvh_list):
    ...
    while True:
        data = Q_speed_in.get()
        if data["sensitivity_flag"]:
            try:
                # 灵敏度校准
                # print(data['param'])
                sensitivity_calc(data['param'], data['gv_dict_status'])
            except Exception:
                qDAQ_logger.error('sensitivity calculation error, error msg:' + traceback.format_exc())
                data['gv_dict_status']["code"] = 3000
                data['gv_dict_status']["msg"] = "传感器灵敏度校准出错!"
        else:
            ...
            try:
                ...
                gv_dict_flag[flag_index_dict["speed_error"]] = 1
                # 进程结束的信息，在cmd=4时会获取该信息（表示执行完毕一次测试）
        Q_speed_out.put("speed_process")
        del data


def sensitivity_calc(sensitivity_param, gv_dict_status):
    global target_device, target_ai_device, target_ai_info, target_input_mode, target_mode_channels
    # 灵敏度计算进程
    p = 2 * 10 ** (sensitivity_param['sPL'] / 20 - 5)
    gv_dict_status_temp = dict()
    taskinfo = task_info_update(sensitivity_param)
    try:
        if board.lower() == "ni":
            try:
                ...
                dtask.start_task()
            frame_counter = 0
            qDAQ_logger.info('NI device initial')
        elif board.lower() == "dt":
            # create a task to read the raw data
            channel_count = len(taskinfo['channelNames'])
            frame_counter = 0
            data_reading_counter = 0
            buffer_counter = 0
            buffer_factor_value, frame_data_len, buffer_len, buffer_data = buffer_creator(taskinfo)
            target_ai_device = set_daq_task(taskinfo, target_ai_info, target_ai_device,
                                            buffer_factor_value, buffer_data,
                                            target_mode_channels, target_input_mode)
            qDAQ_logger.info('DT device initial')
    except Exception:
        qDAQ_logger.error('Error msg:' + traceback.format_exc())
    else:
        data_array = []
        data_leng = sensitivity_param['sampleRate'] * sensitivity_param['time']  # 默认2秒钟的长度
    while gv_dict_flag[flag_index_dict["calibration"]]:
        # 采集数据
        # qDAQ_logger.info('calibrate start read data at {}'.format(time.time()))
        if board.lower() == "ni":
            data = np.array(dtask.read_data())
            frame_counter += 1
        elif board.lower() == "dt":
            frame_counter_bak = frame_counter
            while frame_counter == frame_counter_bak:
                # 未读到一帧数据，所以这里至少会读一次（但一次的数据可能不够一帧）
                    ...
                    if transfer_status.current_total_count - frame_counter * frame_data_len <= buffer_len:
                        ...
                        if rest_part < frame_data_len:
                            ...
                            for chan_index in range(channel_count):
                                data = temp_data[chan_index:: channel_count]
                            # buffer_counter += 1
                        else:
                            ...
                            for chan_index in range(channel_count):
                                data = buffer_data[start_index + chan_index: end_index: channel_count]
                        frame_counter += 1
                    else:
                        raise ValueError("data overflow")
        # qDAQ_logger.info('calibrate start calc data at {}'.format(time.time()))
        data_array.extend(data)
        # qDAQ_logger.info('calibrate start put data at {}'.format(time.time()))
        if len(data_array) >= data_leng:
            gv_dict_status_temp['sensitivity'] = gv_dict_status['sensitivity']
            # qDAQ_logger.info('1 at {}'.format(time.time()))
            gv_dict_status_temp['sensitivity']['sensitivity'].append(
                '{:.2f}'.format((cal_sensity(p, data_array[:data_leng]))))  # 只取2s数据
            # qDAQ_logger.info('2 at {}'.format(time.time()))
            gv_dict_status_temp['sensitivity']['rawData'].append(data_array[data_leng - 200:data_leng])
            # qDAQ_logger.info('3 at {}'.format(time.time()))
            gv_dict_status['sensitivity'] = gv_dict_status_temp['sensitivity']
            # 去掉已经计算完的数据
            data_array = data_array[data_leng:]
        # qDAQ_logger.info('calibrate end calc data at {}'.format(time.time()))
    if board.lower() == "ni":
        dtask.save_task()
        dtask.stop_task()
        dtask.clear_task()
...
def rawdataConsumer_for_qdaq(Q_speed_nvh_list, param, gv_dict_status,
    ...
    qDAQ_logger.debug(f"rawdataConsumer:pid={os.getpid()},ppid={os.getppid()},thread={threading.current_thread().name}")

    gv_dict_status_temp = dict()
    # 获取通道名称信息
    channelNames = param.taskInfo['channelNames']
    testSectionNum = len(param.speedCalcInfo)
    if running_mode == "rt" and board.lower() == "ni":
        # create a task to read the raw data
        try:
            ...
            dtask.createtask()
            dtask.start_task()
        qDAQ_logger.debug(f"NI task start，type:{param.basicInfo['type']},serialNo:{param.basicInfo['serialNo']}")
        ...
        target_ai_device = set_daq_task(param.taskInfo, target_ai_info, target_ai_device,
                                        buffer_factor_value, buffer_data,
                                        target_mode_channels, target_input_mode)
        qDAQ_logger.debug(f"DT task start，type:{param.basicInfo['type']},serialNo:{param.basicInfo['serialNo']}")
        ...
        qDAQ_logger.info(f"time after read all simu rawdata:{time.time() - time_click_start}")
    ...
    vibindex = 0
    for channelName in param.taskInfo['channelNames']:
        if channelName.lower()[:3] in ('mic', 'umi', 'vib'):  # 记录传感器信号原始数据
            gv_dict_rawdata[channelName] = np.ndarray(shape=(max_size,), dtype="f",
                                                      buffer=shm_vib[vibindex].buf,
                                                      offset=0)
            vibindex += 1
        # elif channelName.lower()[:3] in ('spe', 'sin', 'cos'):  # 非传感器信号原始数据
        #     gv_dict_rawdata[channelName] = np.zeros(shape=(max_size,))
        else:  # 记录非传感器信号之外的信号，如sin,cos或者torque
            ...
            if running_mode == "rt" and board.lower() == "ni":
                    ...
                    if transfer_status.current_total_count - frame_counter * frame_data_len <= buffer_len:
                        ...
                        if rest_part < frame_data_len:
                            ...
                            for chan_index in range(channel_count):
                                ...
                                index_rawdata:sampsPerChan + index_rawdata] = temp_data[chan_index:: channel_count]
                            # buffer_counter += 1
                        else:
                            # do not need to take data from start and end
                            start_index = (frame_counter * frame_data_len) % buffer_len
                ...
                if index_rawdata + sampsPerChan <= simu_data_len:
                    for channelName in channelNames:
                        ...
                        gv_dict_rawdata[channelName][index_rawdata:index_rawdata + sampsPerChan] = cdata[channelName]
                        # 控制simu模式的数据读取速度
                        time.sleep(simu_sleep_time)
                    index_rawdata += sampsPerChan
                else:
                    gv_dict_flag[flag_index_dict["speed_finish"]] = 1
            if index_rawdata_backup == index_rawdata:  # 没有读到数据则再重复进行
                continue
            if speedCalFlag:
                if counter == 0:
                    last_angle_l1f, loc_l1f = 0, None
                ...
                try:
                    if speedSignals[recog_index] == "ttl":
                        ...
                        temptl, last_angle_l1f, loc_l1f, last_angle_l2f, loc_l2f, counter_angle = resolver(
                            ...
                            counter_angle,
                            sampsPerChan)
                        trigger_array[index_trigger:index_trigger + len(temptl)] = np.array(
                            temptl) + index_rawdata_backup
                        index_trigger += len(temptl)

                    rpm_index_backup = rpm_index
                    last_rpm_cal_index, rpm_index, is_first_speed_calc = rpm_calc_for_share(
                        trigger_array,
                        last_rpm_cal_index,
                        index_trigger - 1,
                        sampleRate,
        ...
        if not gv_dict_speedRecog['speedRecogFinish']:
            ...
            if not gv_dict_speedRecog['startFlag']:
                ...
                if gv_dict_speedRecog['startFlag']:
                    ...
                    trigger_start_index = (gv_dict_speedRecog['startpoint_index'] - index_rpm_last_speed_signal) * (
                    ...
                    if gv_dict_speedRecog['endpoint_loc']:
                        ...
                        if param.speedRecogInfo["notDummyFlag"][recog_index]:
                            ...
                            gv_dict_status_temp['data'] = gv_dict_status['data']
                            gv_dict_status_temp["data"]["startX"].append(float(gv_dict_speedRecog["startpoint_loc"]))
                            gv_dict_status_temp["data"]["startY"].append(float(gv_dict_speedRecog["startpoint_speed"]))
                            gv_dict_status_temp["data"]["endX"].append(float(gv_dict_speedRecog["endpoint_loc"]))
                            gv_dict_status_temp["data"]["endY"].append(float(gv_dict_speedRecog["endpoint_speed"]))
                        ...
                        if recog_index < len(param.speedRecogInfo['testName']):
                            ...
                            qDAQ_logger.info(
                                'speed recog finished at: ' + datetime.datetime.now().strftime("%Y%m%d%H%M%S.%f"))
                            gv_dict_speedRecog['speedRecogFinish'] = True
                    else:
                ...
                if param.speedRecogInfo["torqueRecogFlag"][recog_index]:
                    gv_dict_speedRecog = speed_detect_end_with_torque(temp_rpm, rpml_array[:rpm_index],
                                                                      rpm_array[:rpm_index],
                                                                      gv_dict_rawdata['Torque'][:index_rawdata],
                                                                      recog_index, gv_dict_speedRecog,
                                                                      param.speedRecogInfo, sampleRate)
                else:
                    gv_dict_speedRecog = speed_detect_end(temp_rpm, rpml_array[:rpm_index], rpm_array[:rpm_index],
                ...
                if gv_dict_speedRecog['endpoint_loc']:
                    ...
                    if param.speedRecogInfo["notDummyFlag"][recog_index]:
                        ...
                        gv_dict_status_temp["data"]["endX"].append(float(gv_dict_speedRecog["endpoint_loc"]))
                        gv_dict_status_temp["data"]["endY"].append(float(gv_dict_speedRecog["endpoint_speed"]))
                        gv_dict_status_temp["data"]["testName"].append(param.speedRecogInfo["testName"][recog_index])
                        gv_dict_status['data'] = gv_dict_status_temp['data']
                    else:
                        sectionEnd_for_put = True
            ...
            if not None_for_put:
                ...
                if sectionEnd_for_put:
                    ...
                    # if testNameIndex_for_put + 1 == param.speedCalcInfo["switchSection"]: #todo 切换测试段
                    ...
                    if testNameIndex_for_put + 1 == param.speedRecogInfo["test_count_except_dummy"]:
                        ...
                        qDAQ_logger.info(f"speed process consumer section end :{time.time() - time_click_start}")
                        gv_dict_speedRecog['speedRecogFinish'] = True
                    sectionEnd_for_put = False
...
def rawdataConsumer_for_fluctuation_norsp(Q_speed_nvh_list, param, gv_dict_status,
                                          file_info, start_timestamp,
                                          time_click_start):
    global gv_dict_flag, shm_vib, ftp_queue
    global target_device, target_ai_device, target_ai_info, target_input_mode
    global running_mode
    # 设置自动垃圾回收
    gc.enable()
    gc.set_threshold(1, 1, 1)
    ...
    elif running_mode == "rt" and board.lower() == "umic":
