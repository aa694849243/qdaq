...
def order_spectrum_calc_info_update(order_spectrum_calc_info, speed_calc_info, speed_recog_info,
    ...
    if order_spectrum_calc_info['maxOrder'] > max_order_available:
        raise ValueError(
            "max order: {} set error, should less than {}".format(order_spectrum_calc_info['maxOrder'],
                                                                  max_order_available))
    if order_spectrum_calc_info['overlapRatio'] >= 1:
        order_spectrum_calc_info['overlapRatio'] = 0
    # 定义角度域降采样之前的采样点角度间隔
    order_spectrum_calc_info['dr_bf'] = speed_recog_info['overallMinSpeed'] / 60 / task_info[
        'sampleRate']
    # 定义角度域降采样之后的采样点角度间隔
    ...
    order_spectrum_calc_info['nstep'] = int(order_spectrum_calc_info['nfft'] *
                                            (1 - order_spectrum_calc_info[
                                                'overlapRatio']))
    # 构建窗函数
    if len(order_spectrum_calc_info['window']) != len(task_info['sensorChan']):
...
def time_freq_map_calc_info_update(time_freq_map_calc_info):
    ...
    time_freq_map_calc_info['nstep'] = time_freq_map_calc_info['nperseg'] - \
                                       time_freq_map_calc_info['noverlap']
    if time_freq_map_calc_info['window']:
        time_freq_map_calc_info['win'], _ = filter_win(
...
class Parameters:
    ...
    def __init__(self, filename):
        ...
        if self.taskInfo["speedFlag"] or (version in [3, 4, 5]):
            ...
            # 确认转速信号信息
            self.speedCalcInfo = speed_calc_info_update(
                speed_calc_info=data["speedCalcInfo"])
            self.speedRecogInfo = speed_recog_info_update(
                speed_recog_info=data["speedRecogInfo"])
            self.timeDomainCalcInfo = time_domain_calc_info_update(
                data["timeDomainCalcInfo"], self.taskInfo,
                self.basicInfo)
            self.orderSpectrumCalcInfo = order_spectrum_calc_info_update(
                data["orderSpectrumCalcInfo"],
                self.speedCalcInfo, self.speedRecogInfo, self.taskInfo)
            self.orderCutCalcInfo = order_cut_calc_info_update(
                data["orderCutCalcInfo"],
                data["orderSpectrumCalcInfo"])
            self.onedOSCalcInfo = oned_os_calc_info_update(data["onedOSCalcInfo"],
                                                           data[
                                                               "orderSpectrumCalcInfo"])
            self.modulationDepthCalcInfo = data["modulationDepthCalcInfo"]
            self.cepstrumCalcInfo = cepstrum_calc_info_update(
                data['orderSpectrumCalcInfo'])
            ...
            self.timeFreqMapCalcInfo = time_freq_map_calc_info_update(
                data["timeFreqMapCalcInfo"])
            self.ssaCalcInfo = ssa_calc_info_update(data["ssaCalcInfo"],
                                                    self.speedCalcInfo)
            self.statFactorCalcInfo = stat_factor_calc_info_update(
                data['indicatorByRevCalcInfo'], self.ssaCalcInfo,
            ...
            self.ftpUploadInfo[
                'dataStatus'] = "http://" + server_ip + ":8081/api/storage/originData/status"
            self.ftpUploadInfo[
                'dataAnalysis'] = "http://" + server_ip + ":8081/api/storage/originData/save"
            # 系统信息暂时不用获取
            self.ftpUploadInfo[
                'systemGet'] = "http://" + server_ip + ":8081/api/storage/system/getSystemName"
