...
def create_empty_tempsf_for_share(stat_factor_calc_info, rspMaxLen):
    """
    功能：初始化按圈计算的统计学指标临时结果，用于最后计算一维指标
    输入：
    1. 计算参数信息
    ...
    """
    tempsf_result = dict()
    if not set(stat_factor_calc_info['indicatorList']):
        return tempsf_result
...
def create_empty_tempsf_for_const(stat_factor_calc_info, max_circle):
    """
